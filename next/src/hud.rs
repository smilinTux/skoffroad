// HUD overlay for SandK Offroad — built with Bevy 0.18 built-in UI.
//
// Layout:
...
impl Plugin for HudPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.init_resource::<HudVisible>()
            ...
            .add_systems(
                Update,
                (update_hud, toggle_hud).run_if(resource_exists::<VehicleRoot>),
            );
    }
}
...
fn spawn_hud(mut commands: Commands) {
    ...
    // Attach panels to root
    commands
        .entity(root)
        .add_children(&[tl_panel, br_panel]);
}
