...
// ~/.skoffroad/benchmark-{ts}.txt with min/max/avg/p1/p99 frame times.
//
// Public API:
//   BenchmarkPlugin
...
fn bench_report_path(unix_ts: u64) -> PathBuf {
    ...
    // Always write to ~/.skoffroad/ regardless of platform.
    match std::env::var("HOME").ok().map(PathBuf::from) {
        Some(home) => home.join(".skoffroad").join(filename),
        None => {
