...
//                           (Sprint 87: procedural surface textures on wood/rock props)
//
...
// Sprint 87 changes
// -----------------
// Surface textures applied via a PostStartup system that reads PropTextures
// (generated by PropTexturesPlugin in Startup).  The system scans all
// StandardMaterial assets and identifies wood/rock ones by colour+roughness
// heuristics, then applies base_color_texture.  On Medium/High:
//   - wood props (signs, posts, cabin walls) get wood_grain texture
//   - rock props (cairns, boulders, dirt barriers) get rough_rock texture
// Low tier keeps plain-colour materials (PropTextures absent, no GPU gen).
//
...
impl Plugin for MapDressingPlugin {
    fn build(&self, app: &mut App) {
        ...
        // PostStartup: apply procedural textures to the wood/rock materials
        // once PropTextures has been inserted by PropTexturesPlugin (Startup).
        .add_systems(PostStartup, apply_prop_surface_textures);
    }
}
...
// ---------------------------------------------------------------------------
// Sprint 87 — PostStartup texture upgrade
// ---------------------------------------------------------------------------

/// Runs once in PostStartup, after PropTexturesPlugin's Startup system has
/// inserted PropTextures.  Scans all StandardMaterial assets and applies:
///   - wood_grain  to materials that look like wood (brown, rough, no emissive,
///                 no metallic, no existing texture)
///   - rough_rock  to materials that look like stone/granite
///
/// Uses the same scan-and-upgrade pattern as vehicle_textures.rs.
/// If PropTextures is absent (Low tier / headless), this system is a no-op.
fn apply_prop_surface_textures(
    prop_textures: Option<Res<PropTextures>>,
    mut materials: ResMut<Assets<StandardMaterial>>,
) {
    let Some(textures) = prop_textures else {
        // Low tier / headless — no textures generated, keep plain colours.
        return;
    };

    let wood_h = textures.wood_grain.clone();
    let rock_h = textures.rough_rock.clone();

    let mut wood_upgraded = 0usize;
    let mut rock_upgraded = 0usize;

    // Collect all asset ids first to avoid borrow issues.
    let ids: Vec<AssetId<StandardMaterial>> = materials.ids().collect();

    for id in ids {
        let Some(mat) = materials.get_mut(id) else { continue };

        // Skip if already has a base_color_texture (don't double-apply).
        if mat.base_color_texture.is_some() { continue; }
        // Skip emissive materials (flags, flames, signs — keep their glow).
        if mat.emissive.red > 0.05 || mat.emissive.green > 0.05 || mat.emissive.blue > 0.05 {
            continue;
        }
        // Skip metallic materials (poles, towers, weather station).
        if mat.metallic > 0.05 { continue; }
        // Skip transparent materials (window glass).
        if !matches!(mat.alpha_mode, AlphaMode::Opaque) { continue; }

        let c = mat.base_color.to_srgba();

        // --- Wood detection: brown-ish, high red, moderate green, low blue ---
        // Matches WOOD_COLOR (0.45,0.30,0.18), LOG_BROWN (0.40,0.28,0.16),
        // fire_log (~0.30,0.18,0.08), sign_post (WOOD_COLOR).
        let is_wood = c.red > 0.25
            && c.red > c.blue * 1.8
            && c.green > c.blue
            && c.green < c.red
        ...
        if is_wood {
            mat.base_color_texture = Some(wood_h.clone());
            wood_upgraded += 1;
            continue;
        }

        // --- Rock detection: neutral grey, high roughness, r≈g≈b ---------
        // Matches GRANITE_COLOR (0.52,0.50,0.48), DIRT_COLOR (0.38,0.26,0.14).
        let is_rock = c.red > 0.30
            && c.red < 0.70
            && (c.red - c.green).abs() < 0.15
            && (c.red - c.blue).abs() < 0.20
            && mat.perceptual_roughness > 0.93;

        if is_rock {
            mat.base_color_texture = Some(rock_h.clone());
            rock_upgraded += 1;
        }
    }

    if wood_upgraded + rock_upgraded > 0 {
        info!(
            "map_dressing: applied prop surface textures -- wood: {}, rock: {}",
            wood_upgraded, rock_upgraded
        );
    }
}
